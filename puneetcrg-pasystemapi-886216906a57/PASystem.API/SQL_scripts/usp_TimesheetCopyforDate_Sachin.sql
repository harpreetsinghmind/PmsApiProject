...
/****** Object:  StoredProcedure [dbo].[usp_TimesheetCopyforDate]    Script Date: 20-12-2019 11:41:03 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROC [dbo].[usp_TimesheetCopyforDate]
    @SourceDate DATETIME,  
...
AS   
 ...
 BEGIN TRAN
	...
	DECLARE @OldtimesheetId BIGINT, @ProjectId BIGINT, @PhaseId BIGINT, @SubPhaseId BIGINT, @TaskId BIGINT, @SubTaskId BIGINT
	...
	BEGIN
		...
		FETCH NEXT FROM copytimesheetforday_Cursor     
				INTO @OldtimesheetId,@ProjectId,@PhaseId,@SubPhaseId,@TaskId,@SubTaskId
				...
				BEGIN 
					IF EXISTS(SELECT Id FROM [dbo].[Timesheet] WITH (UPDLOCK) 
								WHERE EmployeeId = @EmployeeId AND  ProjectId = @ProjectId AND PhaseId = @PhaseId AND SubPhaseId = @SubPhaseId
										AND TaskId = @TaskId AND (@SubTaskId IS NULL OR SubTaskId = @SubTaskId)
						...
						BEGIN
							...
							INSERT INTO [dbo].[Timesheet]
										...
										,[Day1],[Day2],[Day3],[Day4],[Day5],[Day6],[Day7],[ODay1],[ODay2],[ODay3],[ODay4],[ODay5]
										,[ODay6],[ODay7],[DDay1],[DDay2],[DDay3],[DDay4],[DDay5],[DDay6],[DDay7],[Billing],[CreatedBy],[CreatedDate])
								SELECT @EmployeeId,[ProjectId],[PhaseId],[SubPhaseId],[TaskId],[SubTaskId]
...
ELSE
	BEGIN
		SELECT @ProjectId = ProjectId, @PhaseId =  PhaseId, @SubPhaseId = SubPhaseId, @TaskId = TaskId, @SubTaskId = SubTaskId
		FROM [dbo].[Timesheet] WITH (UPDLOCK)
		WHERE EmployeeId = @EmployeeId AND 
			(CONVERT(VARCHAR(20),[DDay1],111) = CONVERT(VARCHAR(20),@SourceDate,111) OR   
			CONVERT(VARCHAR(20),[DDay2],111) = CONVERT(VARCHAR(20),@SourceDate,111) OR   
		...
		IF NOT EXISTS(SELECT Id FROM [dbo].[Timesheet] WITH (UPDLOCK)
						WHERE EmployeeId = @EmployeeId AND ProjectId = @ProjectId AND PhaseId = @PhaseId AND SubPhaseId = @SubPhaseId
						AND TaskId = @TaskId AND (@SubTaskId IS NULL OR SubTaskId = @SubTaskId)
							AND (CONVERT(VARCHAR(20),[DDay1],111) = CONVERT(VARCHAR(20),@DestinationDate,111) OR   
								CONVERT(VARCHAR(20),[DDay2],111) = CONVERT(VARCHAR(20),@DestinationDate,111) OR   
