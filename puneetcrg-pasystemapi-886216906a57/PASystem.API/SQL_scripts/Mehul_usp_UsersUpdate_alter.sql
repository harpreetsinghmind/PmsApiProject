...
/****** Object:  StoredProcedure [dbo].[usp_UsersUpdate]    Script Date: 4/12/2019 1:22:32 PM ******/
SET ANSI_NULLS ON
GO
...
AS 
	...
	BEGIN TRAN
	IF NOT EXISTS(select * from Users where lower(UserName) = LOWER(@UserName) and UserId <>@UserId)
	BEGIN
	...
	SELECT [UserId], [Email],[UserName], [PasswordHash], [SecurityStamp], [EmailConfirmed], [InActive], [UpdatedBy], [UpdatedDate]
	FROM   [dbo].[Users]
	WHERE  [UserId] = @UserId	
