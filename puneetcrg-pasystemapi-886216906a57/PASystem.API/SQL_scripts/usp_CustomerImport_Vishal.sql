ALTER PROC [dbo].[usp_CustomerImport] 
					...
					BEGIN
						IF @IsDeleted = 0
							BEGIN
								IF @allowAdd = 1
									BEGIN
									IF @ContactPersonEmail IS NOT NULL AND @ContactPersonMobileNo IS NOT NULL AND @ContactPersonEmail <> '' AND @ContactPersonMobileNo <> ''
											BEGIN
						...
						ELSE
							BEGIN
								IF @AllowEdit = 1
									...
									--SET @OldCustomerId = (SELECT TOP 1 CustomerId FROM [Customers] WHERE LOWER(CustomerName) = LOWER(@CustomerName))
									...
									IF @ContactPersonEmail IS NOT NULL AND @ContactPersonMobileNo IS NOT NULL AND @ContactPersonEmail <> '' AND @ContactPersonMobileNo <> ''
										...
										SELECT TOP 1 CustomerId FROM [Customers] WHERE LOWER(CustomerName) = LOWER(@CustomerName) --UPDATE
									END
								ELSE
									BEGIN
										SELECT -5
