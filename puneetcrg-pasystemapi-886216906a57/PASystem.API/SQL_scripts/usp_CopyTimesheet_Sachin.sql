...
/****** Object:  StoredProcedure [dbo].[usp_CopyTimesheet]    Script Date: 20-12-2019 11:42:45 ******/
SET ANSI_NULLS ON
GO
...
AS 
	...
	BEGIN TRY 
		...
		DECLARE Timesheet_CURSOR CURSOR FAST_FORWARD FOR  
			SELECT [Id],[ProjectId],[PhaseId],[SubPhaseId],[TaskId],[SubTaskId],
				[Day1],[Day2],[Day3],[Day4],[Day5],[Day6],[Day7],
				[ODay1],[ODay2],[ODay3],[ODay4],[ODay5],[ODay6],[ODay7],
				[DDay1],[DDay2],[DDay3],[DDay4],[DDay5],[DDay6],[DDay7],
				[Billing],
			...
			WHERE EmployeeId = @EmployeeId AND DDay1 = @SDate AND DDay7 = @EDate

			DECLARE @timesheetId BIGINT, @ProjectId BIGINT, @ProjectPhaseId BIGINT, @ProjectSubPhaseId BIGINT, @TaskId BIGINT, @SubTaskId BIGINT,
					@Day1 DECIMAL(10,2), @Day2 DECIMAL(10,2), @Day3 DECIMAL(10,2), @Day4 DECIMAL(10,2), @Day5 DECIMAL(10,2), @Day6 DECIMAL(10,2), @Day7 DECIMAL(10,2),
					@ODay1 DECIMAL(10,2), @ODay2 DECIMAL(10,2), @ODay3 DECIMAL(10,2), @ODay4 DECIMAL(10,2), @ODay5 DECIMAL(10,2), @ODay6 DECIMAL(10,2), @ODay7 DECIMAL(10,2),
					@DDay1 DATETIME, @DDay2 DATETIME, @DDay3 DATETIME, @DDay4 DATETIME, @DDay5 DATETIME, @DDay6 DATETIME, @DDay7 DATETIME,
					@Billing INT, @UpdateDate DATETIME, @Locked BIT
		OPEN Timesheet_CURSOR  
		FETCH NEXT FROM Timesheet_CURSOR INTO @timesheetId, @ProjectId, @ProjectPhaseId, @ProjectSubPhaseId, @TaskId, @SubTaskId,
			@Day1, @Day2, @Day3, @Day4, @Day5, @Day6, @Day7, 
			@ODay1, @ODay2, @ODay3, @ODay4, @ODay5, @ODay6, @ODay7,  
			@DDay1, @DDay2, @DDay3, @DDay4, @DDay5, @DDay6, @DDay7, 
			@Billing, @UpdateDate, @Locked
		...
		BEGIN 
			...
			--IF @ProjectId IS NULL AND @ProjectPhaseId IS NULL AND @ProjectSubPhaseId IS NULL AND @TaskId IS NULL
			--BEGIN
			--	SELECT @updateTimesheetId = Id FROM Timesheet WHERE DDay1 = DATEADD(dd,7,@DDay1) AND DDay7 = DATEADD(dd,7,@DDay7) AND EmployeeId = @EmployeeId 
			--		AND ProjectId IS NULL AND PhaseId IS NULL AND SubPhaseId IS NULL AND TaskId IS NULL
			--END
			--ELSE IF @ProjectPhaseId IS NULL AND  @ProjectSubPhaseId IS NULL AND @TaskId IS NULL
			--BEGIN
			--	SELECT @updateTimesheetId = Id FROM Timesheet WHERE DDay1 = DATEADD(dd,7,@DDay1) AND DDay7 = DATEADD(dd,7,@DDay7) AND EmployeeId = @EmployeeId
			--		AND ProjectId = @ProjectId AND PhaseId IS NULL AND SubPhaseId IS NULL AND TaskId IS NULL
			--END
			--ELSE IF @ProjectSubPhaseId IS NULL AND @TaskId IS NULL
			--BEGIN
			--	SELECT @updateTimesheetId = Id FROM Timesheet WHERE DDay1 = DATEADD(dd,7,@DDay1) AND DDay7 = DATEADD(dd,7,@DDay7) AND EmployeeId = @EmployeeId
			--		AND ProjectId = @ProjectId AND PhaseId = @ProjectPhaseId AND SubPhaseId IS NULL AND TaskId IS NULL
			--END
			--ELSE
			--BEGIN
			...
			SELECT @updateTimesheetId = Id FROM Timesheet WHERE DDay1 = DATEADD(dd,7,@DDay1) AND DDay7 = DATEADD(dd,7,@DDay7) AND EmployeeId = @EmployeeId
				AND (@ProjectId IS NULL OR ProjectId = @ProjectId) AND (@ProjectPhaseId IS NULL OR PhaseId = @ProjectPhaseId)
				AND (@ProjectSubPhaseId IS NULL OR SubPhaseId = @ProjectSubPhaseId) AND (@TaskId IS NULL OR TaskId = @TaskId) AND (@SubTaskId IS NULL OR SubTaskId = @SubTaskId)

			DECLARE @CurDate DATETIME = GETDATE()
			
			IF @updateTimesheetId IS NULL
			BEGIN 
				DECLARE @IDay1 DECIMAL(10,2) = 0,
				...
				INSERT INTO [dbo].[Timesheet]
					([EmployeeId],[ProjectId],[PhaseId],[SubPhaseId],[TaskId],[SubTaskId],
					[Day1],[Day2],[Day3],[Day4],[Day5],[Day6],[Day7],
					[ODay1],[ODay2],[ODay3],[ODay4],[ODay5],[ODay6],[ODay7],
					[DDay1],[DDay2],[DDay3],[DDay4],[DDay5],[DDay6],[DDay7],
					[Billing],[CreatedBy],[CreatedDate])
				VALUES 
					(@EmployeeId,@ProjectId,@ProjectPhaseId,@ProjectSubPhaseId,@TaskId,@SubTaskId
					...
					,DATEADD(dd,7,@DDay1),DATEADD(dd,7,@DDay2),DATEADD(dd,7,@DDay3)
					,DATEADD(dd,7,@DDay4),DATEADD(dd,7,@DDay5),DATEADD(dd,7,@DDay6)
					,DATEADD(dd,7,@DDay7)
					,@Billing,@CreatedBy,GETDATE())
				...
				IF(@IsCopyEverything = 1)
					BEGIN
						...
						(Day,Detail,WeekDay,Status,Hours,OHours,BillingNote,CreatedBy,CreatedDate,TimesheetId)
					SELECT 
						(CASE 
						...
						END) AS Day
						,Detail,WeekDay,Status,Hours,OHours,BillingNote,@CreatedBy,GETDATE(),@newTimesheetId
			...
			FETCH  FROM Timesheet_CURSOR INTO  @timesheetId, @ProjectId, @ProjectPhaseId, @ProjectSubPhaseId, @TaskId, @SubTaskId, 
												@Day1, @Day2, @Day3, @Day4, @Day5, @Day6, @Day7, @ODay1, @ODay2, @ODay3, @ODay4, @ODay5, @ODay6, @ODay7,
												@DDay1,  @DDay2,  @DDay3,  @DDay4, @DDay5, @DDay6, @DDay7, @Billing, @UpdateDate, @Locked
