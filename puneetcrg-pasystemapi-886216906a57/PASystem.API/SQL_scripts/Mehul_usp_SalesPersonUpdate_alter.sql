...
/****** Object:  StoredProcedure [dbo].[usp_SalesPersonUpdate]    Script Date: 4/11/2019 2:20:37 PM ******/
SET ANSI_NULLS ON
GO
...
AS 
	...
	BEGIN TRAN
	IF NOT EXISTS(select * from SalesPerson where lower(WorkEmail) = LOWER(@WorkEmail) and SalesPersonId <>@SalesPersonId)
	BEGIN
		IF NOT EXISTS(select * from SalesPerson where MobileNo = @MobileNo and SalesPersonId <>@SalesPersonId)
		BEGIN
